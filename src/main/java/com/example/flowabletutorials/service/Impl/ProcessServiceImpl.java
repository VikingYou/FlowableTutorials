...
import com.example.flowabletutorials.service.ProcessService;
...
public class ProcessServiceImpl implements ProcessService {
    ...
    protected Process convertBasicHistoricProcessInstance(HistoricProcessInstance historicProcessInstance, Process process) {
        ...
        process.setName(historicProcessInstance.getDescription());
    ...
    @Override
    public List<ProcessInstance> queryProcessByStarter(String starter) {
        return runtimeService.createProcessInstanceQuery()
                .startedBy(starter)
                .list();
    }
    ...
    public List<TaskInfo> queryTasksInProcess(String pid) {
        return taskService.createTaskQuery()
                .processInstanceId(pid).orderByTaskCreateTime()
                .desc().list().stream().map(this::convertTask).collect(Collectors.toList());
    }
    ...
    public List<TaskInfo> queryTasksByAssignee(String assignee) {
        return taskService.createTaskQuery()
                ...
                .list()
                .stream()
                .map(this::convertTask).collect(Collectors.toList());
    }
    ...
    private TaskInfo convertTask(Task task) {
        ...
        taskInfo.setProcessDefinitionId(task.getTaskDefinitionId());
